...
class PulseSequence:
    def __init__(self, start_time=0):
        ...
        self.pulse_names = (
            []
        )  # list of tuples, every tuple is the levels b/w which the pulse operates, alphabetically listed
    ...
    def get_pulse_names(self, simplified=False):
        if not simplified:
            return self.pulse_names
        else:
            return list(set(self.pulse_names))
    ...
    def get_drive_qubits(self, simplified=False):
        if not simplified:
            ...
            for i, pulse in enumerate(self.pulse_names):
                if pulse in pulse_to_drive_qubits:
                    continue
    ...
    def get_pulse_amps(self, simplified=False):
        if not simplified:
            return np.array(self.amps)
        else:
            pulse_to_amps = dict()
            for i, pulse in enumerate(self.pulse_names):
                if pulse in pulse_to_amps:
                    continue
    ...
    def get_end_time(self):  # end time of full pulse sequence, including empty wait times
        ...
        for q, pulse_len, start_time in zip(self.get_drive_qubits(), self.get_pulse_lengths(), self.get_start_times()):
            if str(q) not in end_pulse_times_q.keys():
                ...
                end_pulse_times_q.update({str(q): max((end_pulse_times_q[str(q)], start_time + pulse_len))})
    ...
    def const_pulse(
        ...
        amp=None,
        t_pulse=None,
        pulse_levels: Tuple[str, str] = None,
        modulation: Function = None,
        drive_qubit=1,
        t_offset=0,
        t_start=None,
        t_rise=1,
        ...
        if t_start is None:
            t_start = self.time + t_offset
        self.time = t_start + t_pulse

        envelope = None
        drive_func = None
        c_str = None
        if None in pulse_levels:
            pulse_levels = None
        ...
        if amp is not None:
            ...
            def envelope(t, args=None):
                ...
                elif t_rise <= t < t_pulse - t_rise:
                    return 1
                elif t_pulse - t_rise <= t < t_pulse:
                    return np.sin(np.pi * (t_pulse - t) / 2 / t_rise) ** 2
                else:
                    return 0
            ...
            def drive_func(t, args=None):
                if modulation is None:
                    ...
                    return envelope(t) * modulation(t, wd, amp, phase)
            ...
            c_str += (
                f" + (np.heaviside(t-({t_start})-({t_rise}),0)-np.heaviside(t-({t_start})-({t_pulse})-({t_rise}),0))"
            )
    ...
    def gaussian_pulse(
        ...
        if amp is not None:
            ...
            def envelope(t, args=None):
                ...
                if t < t_start or t > t_start + sigma_n * t_pulse_sigma:
                    return 0
    ...
    def adiabatic_pulse(
        ...
        if amp is not None:
            ...
            def envelope(t, args=None):
                if t < t_start or t > t_start + period:
                    return 0
            ...
            def drive_func(t, args=None):
                ...
                return (
                    amp
                    * envelope(t)
                    * (np.cos(phase_t) * np.cos(wd * t + phase) - np.sin(phase_t) * np.sin(wd * t + phase))
                )
    ...
    def DRAG_pulse(
        ...
        def drive_func(t, args):
            return (
                amp * envelope_I(t) * np.cos(wd * t + phase)
                - alpha * amp * envelope_Q(t) * np.sin(wd * t + phase) / delta
            )
        ...
        if amp is not None:
            ...
            if envelope is None:
                ...
                def envelope(t, args=None):
                    ...
                    if 0 <= t < t_rise:
                        return gaussian(t - t_rise, t_ramp_sigma)
                    elif t_rise <= t < t_pulse - t_rise:
                        return 1
                    elif t_pulse - t_rise <= t < t_pulse:
                        return gaussian(t - (t_pulse - t_rise), t_ramp_sigma)
                    else:
                        return 0
    ...
    def pulse_IQ(
        ...
        assert drive_qubit is not None
        if t_start is None:
            t_start = self.time + t_offset
        ...
        if amp is not None:
            ...
            assert pulse_levels is not None
            assert I_values is not None
            assert Q_values is not None
            I_func = sp.interpolate.interp1d(times, I_values, fill_value=0, bounds_error=False, kind="linear")
            Q_func = sp.interpolate.interp1d(times, Q_values, fill_value=0, bounds_error=False, kind="linear")
            envelope = [lambda t, args=None: I_func(t - t_start), lambda t, args=None: Q_func(t - t_start)]

            def drive_func(t, args=None):
                return amp * I_func(t - t_start) * np.cos(wd * t + phase) + amp * Q_func(t - t_start) * np.sin(
                    wd * t + phase
                )
    ...
    def flux_transition(self, t_flux_rise, flux_f, t_begin_transit=None):
        if t_begin_transit is None:
            t_begin_transit = self.time
    ...
    def construct_flux_transition_map(self, sigma_n=6):
        ...
        t_downramp_start = (
            transit_start_times[1] if len(flux_seq) >= 2 else np.inf
        )  # time of beginning ramp to next flux
        ...
        for flux_i in range(len(flux_seq)):
            ...
            def H0_flux_window(
                t_to_i=t_upramp_start,
                t_to_f=t_upramp_end,
                t_from_i=t_downramp_start,
                t_from_f=t_downramp_end,
                args=None,
            ):
